<template>
  <div
    ...
    @mousedown="handleScreenInteractionStart"
    @mouseup="handleScreenInteractionEnd"
    @touchstart.prevent="handleScreenInteractionStart"
    @touchend="handleScreenInteractionEnd"
  >
    <div
      class="player"
    ...
    >
      <!-- チャージバー -->
      <div
        v-if="isChargingJump || currentJumpChargeRatio > 0"
        class="charge-bar-container"
        :style="{
      ...
      >
        <div
          ...
          :style="{ width: currentJumpChargeRatio * 100 + '%' }"
        ></div>
      </div>
    </div>
    ...
    <div
      class="ground"
      :style="{ backgroundPositionX: groundScrollX + 'px' }"
    ></div>

    <!-- 障害物の描画 -->
    <template v-for="obstacleInst in obstacles" :key="obstacleInst.id">
      <div
        v-for="(part, partIndex) in obstacleInst.parts"
        :key="`${obstacleInst.id}-${partIndex}`"
        class="obstacle-part"
        :style="{
...
const obstaclePresets: Record<ObstaclePresetType, ObstaclePreset> = {
  ...
  [ObstaclePresetType.SingleHigh]: {
    ...
    baseTotalHeight: 60,
    canBeFloating: true,
    minYOffset: playerHeight * 0.4,
    maxYOffset: playerHeight * 1.5,
    spawnWeight: 25,
  },
  [ObstaclePresetType.TallWall]: {
    ...
    baseTotalWidth: 55,
    baseTotalHeight: gameContainerHeight * 0.7 - groundHeight,
    spawnWeight: 15,
  },
  [ObstaclePresetType.FloatingIsland]: {
    ...
    baseTotalHeight: 60,
    canBeFloating: true,
    minYOffset: playerHeight * 0.8,
    maxYOffset: gameContainerHeight * 0.5,
    spawnWeight: 10,
  },
  ...
  [ObstaclePresetType.VeryHighTrap]: {
    ...
    baseTotalHeight: 30,
    canBeFloating: true,
    minYOffset: gameContainerHeight * 0.75 - groundHeight,
    maxYOffset: gameContainerHeight * 0.85 - groundHeight,
    spawnWeight: 5,
  },
};
...
// --- ジャンプ処理 ---
function performJump(power: number) {
  // ジャンプ力を引数で受け取るように変更
...
function spawnObstacle() {
  const totalWeight = Object.values(obstaclePresets).reduce(
    (sum, preset) => sum + preset.spawnWeight,
    0
  );
  ...
  for (const type in obstaclePresets) {
    ...
    if (obstaclePresets.hasOwnProperty(T)) {
      // または Object.prototype.hasOwnProperty.call(obstaclePresets, T)
  ...
  if (selectedPresetType === null) {
    console.warn(
      "Fallback to SingleLow as selectedPresetType was null. Check spawnWeights if this is unexpected."
    );
  ...
  if (!preset) {
    console.error(
      `FATAL: Preset for type ${ObstaclePresetType[selectedPresetType]} (enum value ${selectedPresetType}) is undefined in obstaclePresets. Please check obstaclePresets definition.`
    );
  ...
  if (
    preset.canBeFloating &&
    preset.minYOffset !== undefined &&
    preset.maxYOffset !== undefined
  ) {
    yPosition =
      groundHeight + getRandomInRange(preset.minYOffset, preset.maxYOffset);
  }

  console.log(
    `Attempting to spawn preset: ${ObstaclePresetType[selectedPresetType]}`
  );

  const generatedParts = preset.parts.map((blueprint, index) => {
    if (!blueprint) {
      console.error(
        `Blueprint at index ${index} for preset ${
          ObstaclePresetType[selectedPresetType!]
        } is undefined.`
      );
      return {
        offsetX: 0,
        ...
        height: 10,
        color: "magenta",
        error: "Undefined blueprint",
      };
    }

    // widthRange のチェック
    if (
      !blueprint.widthRange ||
      !Array.isArray(blueprint.widthRange) ||
      blueprint.widthRange.length < 2
    ) {
      console.error(
        `Blueprint ${index} (offsetX: ${blueprint.offsetX}) for preset ${
          ObstaclePresetType[selectedPresetType!]
        } has invalid or missing widthRange. Actual value:`,
        blueprint.widthRange
      );
      ...
      return {
        offsetX: blueprint.offsetX || 0,
        offsetY: blueprint.offsetY || 0,
        width: 10,
        height: 10,
        color: "purple",
        error: "Invalid widthRange",
      };
    }
    // heightRange のチェック
    if (
      !blueprint.heightRange ||
      !Array.isArray(blueprint.heightRange) ||
      blueprint.heightRange.length < 2
    ) {
      console.error(
        `Blueprint ${index} (offsetX: ${blueprint.offsetX}) for preset ${
          ObstaclePresetType[selectedPresetType!]
        } has invalid or missing heightRange. Actual value:`,
        blueprint.heightRange
      );
      return {
        offsetX: blueprint.offsetX || 0,
        ...
        height: 10,
        color: "orange",
        error: "Invalid heightRange",
      };
    }
    // colorOptions のチェック
    if (
      !blueprint.colorOptions ||
      !Array.isArray(blueprint.colorOptions) ||
      blueprint.colorOptions.length === 0
    ) {
      console.error(
        `Blueprint ${index} (offsetX: ${blueprint.offsetX}) for preset ${
          ObstaclePresetType[selectedPresetType!]
        } has invalid or missing colorOptions. Actual value:`,
        blueprint.colorOptions
      );
      return {
        offsetX: blueprint.offsetX || 0,
        ...
        height: 10,
        color: "cyan",
        error: "Invalid colorOptions",
      };
    }

    const width = getRandomInRange(
      blueprint.widthRange[0],
      blueprint.widthRange[1]
    );
    const height = getRandomInRange(
      blueprint.heightRange[0],
      blueprint.heightRange[1]
    );
  ...
  const validGeneratedParts = generatedParts.filter(
    (part) => !part.hasOwnProperty("error")
  );

  if (validGeneratedParts.length !== generatedParts.length) {
    console.warn(
      `Some parts were invalid and have been filtered out for preset ${
        ObstaclePresetType[selectedPresetType!]
      }.`
    );
  }
  ...
  if (validGeneratedParts.length === 0 && preset.parts.length > 0) {
    console.error(
      `No valid parts could be generated for preset ${
        ObstaclePresetType[selectedPresetType!]
      }. Skipping obstacle spawn.`
    );
    return;
  }
  ...
  } else if (preset.parts.length > 0) {
    // 元々パーツがあるはずなのに全て無効になった場合
    console.warn(
      `Obstacle instance for ${
        ObstaclePresetType[selectedPresetType!]
      } was not pushed because it had no valid parts after filtering.`
    );
  }
}
...
function gameLoop(currentTime: number) {
  ...
  if (isChargingJump.value) {
    ...
    currentJumpChargeRatio.value = Math.min(
      chargeDuration / jumpChargeTimeMax,
      1
    );
  } else {
  ...
  if (playerY.value > groundHeight || isJumping.value) {
    //重力
  ...
  if (currentTime >= nextSpawnTime) {
    ...
    if (consecutiveSpawnCount < maxConsecutiveSpawns && Math.random() < 0.4) {
      // 40%の確率で連続出現
      // 次の連続出現時間を短く設定
      nextSpawnTime =
        currentTime + consecutiveSpawnInterval + getRandomInRange(-50, 50); // 少し間隔を揺らす
    } else {
      // 通常の出現間隔を設定し、連続出現カウントをリセット
      const interval =
        baseObstacleSpawnIntervalMin +
        Math.random() *
          (baseObstacleSpawnIntervalMax - baseObstacleSpawnIntervalMin);
  ...
  for (let i = obstacles.length - 1; i >= 0; i--) {
    ...
    obstacleInst.parts.forEach((part) => {
      // index も不要
    ...
    if (obstacleInst.x + obstacleInst.colliderWidth < 0) {
      // colliderWidth を使用
...
function handleKeyup(event: KeyboardEvent) {
  if (event.code === "Space") {
    if (isChargingJump.value && gameStarted.value && !gameOver.value) {
      ...
      const jumpForce =
        minJumpPower + (maxJumpPower - minJumpPower) * chargeRatio;
...
onMounted(() => {
  ...
  if (gameContainerRef.value) {
    // gameContainerRef を使ったイベントリスナーなど
    gameContainerRef.value.addEventListener(
      "mousedown",
      handleScreenInteractionStart
    );
    gameContainerRef.value.addEventListener(
      "mouseup",
      handleScreenInteractionEnd
    );
    gameContainerRef.value.addEventListener(
      "touchstart",
      handleScreenInteractionStart,
...
onUnmounted(() => {
  ...
  if (gameContainerRef.value) {
    gameContainerRef.value.removeEventListener(
      "mousedown",
      handleScreenInteractionStart
    );
    gameContainerRef.value.removeEventListener(
...
.ground {
  ...
  background-image: linear-gradient(
    to right,
    #654321 50%,
    /* 濃い茶色 */ #8b4513 50% /* 薄い茶色 (SaddleBrown) */
  );
  /* 簡単な縞模様 */
